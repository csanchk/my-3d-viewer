...
/**
 * The event interface of the sdk
 */
...
export interface SetupSdkOptions {
	 ...
	 */
	connectOptions?: Partial<Parameters<MP_SDK["connect"]>>;
	/**
	 * The element or selector that will attach the created iframe.
	 * Defaults to document.body if not specified. Ignored if the `iframe` option is provided.
	 */
	...
	/**
	 * The options to pass to the iframe, as an object of attribute keys and values.
	 *
	 * ```
	...
	/**
	 * The query params to include in the iframe's `src` URL. See https://support.matterport.com/s/article/URL-Parameters
	 * for a full list.
	 *
	 * Do not specify the `m` param in these params - use the `space` option instead.
	 *
	 * ```
	 * { qs: 1 }
	 ...
	 * ```
	 */
	iframeQueryParams?: Record<string, string | number | boolean> | URLSearchParams;
	/**
...
/**
 * Scaffolding function for the SDK. Loads the SDK script, attaches/creates
 * required DOM elements, and loads the desired model.
 *
 * All usage requires an SDK key. See [here](https://matterport.github.io/showcase-sdk/sdk_home.html#how-do-i-obtain-an-sdk-key) for more information.
 *
 * @param sdkKey The SDK key. See [here](https://matterport.github.io/showcase-sdk/sdk_home.html#how-do-i-obtain-an-sdk-key) for more information.
 * @param options Options to pass to the SDK scaffolding (see "Options" [here](https://www.npmjs.com/package/@matterport/sdk#options)). Default `{}`
 * @returns A Promise with the created SDK.
 *
 * #### Minimum setup
 *
 * ```js
 ...
 * // then use the SDK - for example:
 * mpSdk.Camera.turn(45, 0)
 * ```
 *
 * #### Loading a space
 *
 * Retrieve the space's ID: go to [my.matterport.com](my.matterport.com), select your space, and copy the
 * ID after `/models/` and before the query string.
 *
 * Example: `my.matterport.com/models/abcxyz?section=media` has an ID of `abcxyz`.
 *
 * Then:
 * ```js
 * import { setupSdk } from '@matterport/sdk'
 ...
 * ```
 *
 * #### Integrating with an existing UI
 *
 * Set the `container` option as a selector or element:
 *
 * ```
 * import { setupSdk } from '@matterport/sdk'
 * const mpSdk = await setupSdk('YOUR_SDK_KEY', { container: '#showcase-container' })
 * // then use the SDK - for example:
 * mpSdk.Camera.turn(45, 0)
 * ```
 *
 */
export declare const setupSdk: (sdkKey: string, options?: SetupSdkOptions) => Promise<MpSdk>;
